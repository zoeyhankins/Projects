...
(function(window, undefined) {
  ...
  function atPoint(clientX, clientY, lastElement) {
	...
	if(element) {
		if(jQuery(element).is('input:not([readonly]), textarea:not([readonly]), select:not([readonly])')) {
		  ...
		  var list = [];
      	  if($(element).is('.shape.firer')) { list.push($(element)); }
      	  list = list.concat($(element).parentsUntil(".input:not([readonly]), .textarea:not([readonly]), .select:not([readonly])", ".firer").toArray());
      	  return list;
		}
	}
	return [];
  ...
  .bind("touchstart mousedown gesturestart gestureend touchend mouseup touchmove", function(event) {
    var jEvent = jimEvent(event),
        ...
        target = jFirer[0],
        evt = (typeof event.originalEvent === 'undefined') ? event : event.originalEvent,
    ...
    switch(event.type) {
      ...
      case "gesturestart":
        ...
        jFirer.data("jimGesture", {"startX": (hasTouches) ? touches[0].pageX : evt.pageX, "startY": (hasTouches) ? touches[0].pageY : evt.pageY});
        break;
      case "gestureend":
      ...
      case "mouseup":
    	if(jFirer.data("jimStartGesture") && jFirer.data("jimEndGesture")){
            ...
            data = jFirer.data("jimGesture");
        	deltaX = ((hasTouches) ? touches[0].pageX : evt.pageX) - data.startX;
            deltaY = ((hasTouches) ? touches[0].pageY : evt.pageY) - data.startY;
  ...
  window.addEventListener('scroll', function (event) {
    var $firer = $(event.target);
    if ($firer.is("#simulation") || jimDevice.isMobile() && $firer.is(".ui-page")) {
      jQuery(".screen").trigger("scroll.jim");
      jQuery(".template").trigger("scroll.jim");
    } else {
      $firer.trigger("scroll.jim", [{originalFirer:event.target}]);
    }
  }, true);
